...
import type { Page, Guide, Component } from '../payload-types'
    ...
    doc.layout.map(async block => {
      if (block.blockType === 'archive') {
        const archiveBlock = block as Extract<Page['layout'][0], { blockType: 'archive' }> & {
          populatedDocs: Array<{
            relationTo: 'pages' | 'components' | 'guides'
            value: string
          }>
        }

        if (archiveBlock.populateBy === 'collection' && !context.isPopulatingArchiveBlock) {
          const res: { totalDocs: number; docs: (Guide | Component)[] } = await payload.find({
          ...
          return {
            ...
            populatedDocs: res.docs.map((thisDoc: Guide | Component) => ({
